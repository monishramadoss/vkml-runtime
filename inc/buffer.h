...
public:
    // Creation methods
    static std::shared_ptr<Buffer> createStorageBuffer(
        std::shared_ptr<BufferAllocator> allocator,
        VkDeviceSize size,
        VkBufferUsageFlags additionalUsage = 0,
        bool hostVisible = false);

    static std::shared_ptr<Buffer> createUniformBuffer(
        std::shared_ptr<BufferAllocator> allocator,
        VkDeviceSize size,
    ...
    // Buffer views
    BufferView getView(VkDeviceSize offset = 0, VkDeviceSize size = VK_WHOLE_SIZE);
...
private:
    Buffer(std::shared_ptr<BufferAllocator> allocator,
           VkDeviceSize size,
