...
class Device
{
  public:
    ...
    std::shared_ptr<Program> createProgram(const std::vector<uint32_t> &shader, uint32_t dim_x=1, uint32_t dim_y=1,
                                           uint32_t dim_z=1);
    std::shared_ptr<CommandPoolManager> getComputePoolManager(size_t idx = 0);
  ...
  private:
    // Initialize device
    bool initialize(std::shared_ptr<ThreadPool> pool, VkInstance &instance, VkPhysicalDevice &pd);
    void cleanup();
